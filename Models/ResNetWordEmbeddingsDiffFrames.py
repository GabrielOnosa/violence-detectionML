...
class PicturesDatasetEmbeddings(torch.utils.data.Dataset):
    def __init__(self, directory, embeddings_directory, transform=None):
        ...
        for folder in os.listdir(self.directory):
            ...
            for filename in os.listdir(folder_path):
                ...
                row_from_csv = embedding_file.loc[embedding_file['title'] == filename].iloc[0, :-1].values
    ...
    def __getitem__(self, idx):
        ...
        frame_list = Yolo_Frame_selector.yolo_frame_selector(element[0], yolo_pose)
        final_frame = []
        yolo_embeddings = []
...
class EarlyStopping:         # antrenarea se opreste cand apar 2 epoci in care nu mai creste val_loss
    def __init__(self, patience=20, delta=0.001, checkpoint_path="resnet50_embeddings_with_yolo.pth"):
...
if __name__ == '__main__':
    train_dataset = PicturesDatasetEmbeddings(r'D:\Downloads\database\RWF-2000\train', 'gemini_embeds_train_diff_frames.csv', transform = transform)
    t0 = time()
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=32, shuffle = True)
    val_dataset = PicturesDatasetEmbeddings(r'D:\Downloads\database\RWF-2000\val', 'gemini_embeds_val_diff_frames.csv', transform = transform)
    ...
    for epoch in range(nr_epoci):
        train_loss = 0
        val_loss = 0
        for i, (data, text_embedding, pose_embedding, label) in enumerate(
                tqdm(train_dataloader, desc=f"Training Epoch {epoch + 1}", unit="batch")):
            data = data.to("cuda")
            label = label.to("cuda")
